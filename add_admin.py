...
class Main:
    def __init__(self):
        ...
        self.mainLabel = Label(self.root, text=" Add Admin ", font=('calibri', 26, 'bold'), fg ="blue", bg="white", borderwidth=2, relief=RAISED, activebackground="black", activeforeground="white")
        ...
        self.lb5 = Label(self.f, text='Enter Password', font="calibri 15", bg="black", fg="white")
        self.txt5 = Entry(self.f, font=font, width=30, show='*')
    ...
    def submitForm(self):
        ...
        if len(name) == 0 or len(email) == 0 or len(mobile) == 0 or len(role) == 0 or len(password) == 0:
            ...
            if verifyemail(email) == "invalid" or verifymobile(mobile) == "invalid":
                ...
                conn = connect()
                cr = conn.cursor()
                q = f"insert into add_admin values (null, '{name}', '{email}', '{mobile}', '{role}', '{password}')"
